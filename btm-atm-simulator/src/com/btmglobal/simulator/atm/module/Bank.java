...
import com.btmglobal.simulator.atm.data.FileIO;
...
public final class Bank {
	...
	public static void addUser(String firstName, String lastName, byte pinHash[], String uuid, String type) {
		// create a new User object
		if(type.equals("1")) {
			Customer newUser = new Customer(firstName, lastName, pinHash, uuid);
			users.add(newUser);
			// create a savings account
			Account newAccount = new Account("Savings", newUser);
			Account newAccount1 = new Account("Checking", newUser);
			...
			Staff staff = new Staff(firstName, lastName, pinHash, uuid);
			users.add(staff);
		}
	}
	...
	public static void saveData() {
		...
		for (User user : users) {
			userInfo.add(user.toString());
		}
		FileIO.writeToFile(userInfo);
	}

	public static void readData() {
		List<String> fileData = FileIO.readFromFile();
		for (String userInfo : fileData) {
			String firstName = "";
			String lastName = "";
			byte pin[] = new byte[]{};
			String uuid = "";
			String type = "";
			String[] contentArr = userInfo.split(", ");
			for (String str : contentArr) {
				String[] strArr = str.split(": ");
				if (str.contains("firstName")) {
					firstName = strArr[1].trim();
				} else if (str.contains("lastName")) {
					lastName = strArr[1].trim();
				} else if (str.contains("pin")) {
					pin = Base64.getDecoder().decode(strArr[1].trim());
			...
			addUser(firstName, lastName, pin, uuid, type);
		}
	}
